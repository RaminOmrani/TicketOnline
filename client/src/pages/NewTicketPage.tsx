...
import { Mic, Send, Info, Check, Search, Lightbulb } from 'lucide-react';
...
export default function NewTicketPage() {
  ...
  const [dept, setDept] = useState<number>(Number(params.get('department')) || 0);
  const [subject, setSubject] = useState('');
  const [priority, setPriority] = useState<Priority>('normal');
  const [product, setProduct] = useState(settings.products[0] || '');
  ...
  useEffect(() => {
    if (!product && settings.products[0]) setProduct(settings.products[0]);
  }, [settings.products]);
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <form onSubmit={submit} className="mt-5 space-y-5">
        ...
        {/* Department */}
        <div>
          <label className="label">بخش مربوطه <span className="text-rose-500">*</span></label>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {departments.map((d) => (
              <button type="button" key={d.id} onClick={() => setDept(d.id)} className={clsx('card relative flex items-start gap-3 p-4 text-start transition hover:border-brand/50', dept === d.id && 'border-brand ring-4 ring-brand/15')}>
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" style={{ background: `${d.color}1a`, color: d.color || undefined }}><DeptIcon name={d.icon} className="h-6 w-6" /></span>
        ...
        <div className="card space-y-4 p-4 sm:p-6">
          <Field label="موضوع" required error={err('subject')}>
            <input className="input" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="مثلاً: خطا هنگام ثبت سند حسابداری" maxLength={200} dir="auto" />
          </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="محصول / ماژول">
              <select className="input" value={product} onChange={(e) => setProduct(e.target.value)}>
                {settings.products.map((p) => <option key={p} value={p}>{p}</option>)}
              </select>
            </Field>
          </div>
          ...
          <Field label="شرح کامل درخواست" required error={err('body')} hint="نسخه نرم‌افزار، متن خطا و مراحل بروز مشکل را بنویسید. می‌توانید تصویر را مستقیماً Paste کنید.">
            <div className={clsx('relative rounded-xl border bg-white dark:bg-slate-900', err('body') ? 'border-rose-400' : 'border-slate-300 dark:border-slate-700', drag && 'ring-4 ring-brand/20')} onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={(e) => { e.preventDefault(); setDrag(false); e.dataTransfer.files.length && att.addFiles(e.dataTransfer.files); }}>
              ...
              <textarea className="block min-h-[160px] w-full resize-y rounded-xl bg-transparent px-4 py-3 text-sm leading-7 outline-none" value={body} onChange={(e) => setBody(e.target.value)} onPaste={(e) => { const f = Array.from(e.clipboardData.files || []); if (f.length) { e.preventDefault(); att.addFiles(f); } }} dir="auto" placeholder="توضیحات…" />
