...
function NotificationBell() {
  ...
  const { data } = useQuery({ queryKey: ['notifications'], queryFn: () => api.get<{ items: Notification[]; unread: number }>('/notifications?limit=25'), refetchInterval: 60_000 });
      ...
      {(close) => (
        <div>
          ...
          <div className="max-h-96 overflow-y-auto">
            {!data?.items.length ? (
              <EmptyState icon={<Bell />} title="اعلانی ندارید" />
            ) : (
              data.items.map((n) => (
                <button key={n.id} onClick={() => open(n, close)} className={clsx('flex w-full gap-3 rounded-lg px-2.5 py-2.5 text-start transition hover:bg-slate-100 dark:hover:bg-slate-800', !n.is_read && 'bg-brand/5')}>
...
export default function AppShell() {
  ...
  const nav = (
    <nav className="flex flex-1 flex-col gap-1">
      ...
      {isStaff ? (
        ...
        <NavItem to="/tickets" icon={<Ticket />} label="تیکت‌های من" badge={summary?.unread} />
      )}
      <NavItem to="/tickets/new" icon={<PlusCircle />} label="تیکت جدید" />
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar (desktop) */}
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-l border-slate-200 bg-white px-3 py-4 dark:border-slate-800 dark:bg-slate-900 lg:flex">
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-72 flex-col bg-white px-3 py-4 shadow-pop dark:bg-slate-900 animate-fade-in">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Topbar */}
        <header className="sticky top-0 z-30 flex h-16 items-center gap-2 border-b border-slate-200/80 bg-white/80 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/80 sm:px-6">
          ...
          <button className="btn-primary btn-sm hidden sm:inline-flex" onClick={() => navigate('/tickets/new')}>
            <PlusCircle className="h-4 w-4" /> تیکت جدید
          </button>
