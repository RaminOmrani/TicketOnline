...
**همهٔ تنظیمات پیامک از پنل مدیر → تنظیمات → «پیامک» انجام می‌شود** (ارائه‌دهنده، کلید API، کد الگوها و دکمهٔ ارسال تست)؛ نیازی به ویرایش `.env` نیست و مقدار پنل بر `.env` اولویت دارد. در پنل ملی‌پیامک → **خدمات → ارسال با الگو (خدماتی)** هر متن را عیناً ثبت کنید و پس از تأیید، **کد الگو (bodyId)** را در کادر همان ردیف در پنل مدیر (یا در `.env`) بگذارید. متغیرها با `{0}` مشخص می‌شوند.
