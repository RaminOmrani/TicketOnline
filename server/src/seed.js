...
export function seed({ verbose = true } = {}) {
  ...
  if (deptCount === 0) {
    ...
    const depts = [
      ['پشتیبانی فنی', 'technical', 'مشکلات نصب، اجرا، خطاها و راهنمایی استفاده از نرم‌افزار حسابداری میلیاک', 'wrench', '#2563eb', 1, 120, 1440],
      ['مالی و حسابداری', 'finance', 'سوالات حسابداری، صورت‌حساب، فاکتور، تمدید اشتراک و پرداخت', 'calculator', '#16a34a', 2, 240, 2880],
      ['برنامه‌نویسی و توسعه', 'development', 'گزارش باگ، درخواست قابلیت جدید، سفارشی‌سازی، API و یکپارچه‌سازی', 'code', '#7c3aed', 3, 480, 7200],
      ['فروش و تمدید', 'sales', 'خرید نسخه جدید، ارتقا، قیمت‌ها و مشاوره قبل از خرید', 'shopping-bag', '#ea580c', 4, 120, 1440],
      ['آموزش', 'training', 'درخواست آموزش، ویدیوهای آموزشی و راهنمای کار با ماژول‌ها', 'graduation-cap', '#0891b2', 5, 480, 4320],
      ['شکایات و پیشنهادات', 'feedback', 'انتقادات، شکایات و پیشنهادات برای بهبود خدمات', 'message-square-heart', '#db2777', 6, 480, 4320],
    ];
  ...
  if (kbCount === 0) {
    ...
    ins.run(
      'تهیه نسخه پشتیبان از نرم‌افزار حسابداری میلیاک',
      'backup-guide',
